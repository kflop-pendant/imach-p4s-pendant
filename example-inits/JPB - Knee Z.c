...
int main() 
{
	...
	// Screen init readout (Var 170): show "KNEE Z LOADING..." the moment this
	...
	DROLabel(1100, 170, "KNEE Z LOADING...");
	SetUserDataDouble(58, -2.0);	// pendant LCD "<name> / Loading" -- NEGATIVE identity =
									// loading; +2 is published after the TP block (var 58)
...
FPGA(STEP_PULSE_LENGTH_ADD)=32 + 0x40 + 0x80;	
	...
	// "KNEE Z init LOADED" DROLabel is deferred to AFTER the TP block
	...
	// now surfaces as a disconnect message rather than silent corruption.
	{
		...
		// AXIS_C to ch3. Knee Z: ch2 is the knee (427000 cpi), ch3 is the quill
		...
		// of Max Vel. Knee Z: Z=knee 0.08 in/s (4.8 IPM), C=quill 0.3 in/s
		...
		// Screen "KNEE Z init LOADED" indicator, deferred to here so it only
		...
		// the screen shows a distinct TP FAIL message the operator can't miss.
		{
			...
			{
				sprintf(_s, "KNEE Z init LOADED");
				TP_verified = 1;
			}
			else
			{
				sprintf(_s, "KNEE Z TP FAIL!!");
				TP_verified = 0;
			}
	...
	SetUserDataDouble(54, 2.0);	// Knee Z: knee on ch2, quill on ch3
	SetUserDataDouble(58, 2.0);	// pendant init identity -> LCD banner name: 2 = Knee Z  (var 58; NOT 56 = TP_HOLD_REQ)
