...
int main() 
{
	...
	// Screen init readout (Var 170): show "STANDARD LOADING..." the moment this
	...
	DROLabel(1100, 170, "STANDARD LOADING...");
	SetUserDataDouble(58, -1.0);	// pendant LCD "<name> / Loading" -- NEGATIVE identity =
									// loading; +1 is published after the TP block (var 58)
...
FPGA(STEP_PULSE_LENGTH_ADD)=32 + 0x40 + 0x80;
	...
	ch2->MaxI=1e+09;
	ch2->MaxErr=1e+09;
	ch2->MaxOutput=400000;
	ch2->DeadBandGain=0;
	ch2->DeadBandRange=10;
	ch2->InputChan0=2;
	ch2->InputChan1=0;
	ch2->OutputChan0=58;
	ch2->OutputChan1=0;
	...
	ch3->MaxI=120000;
	ch3->MaxErr=1e+09;
	ch3->MaxOutput=250000;
	ch3->DeadBandGain=0;
	ch3->DeadBandRange=25;
	ch3->InputChan0=3;
	ch3->InputChan1=0;
	ch3->OutputChan0=59;
	ch3->OutputChan1=0;
	...
	// "STANDARD init LOADED" DROLabel is deferred to AFTER the TP block
	...
	persist.UserData[150] = 0;	// spindle selector: 0 = big OEM spindle
	...
	// now surfaces as a disconnect message rather than silent corruption.
	{
		...
		printf("SET Z cnts/inch rc=%d  GET Z=%.0f  (want 180000)\n", rcZcpi, vZ);

		rcCcpi = SetTPParameter(PT_COUNTS_PER_INCH, AXIS_C, 427000.0);
		GetTPParameter(PT_COUNTS_PER_INCH, AXIS_C, &vC);
		printf("SET C cnts/inch rc=%d  GET C=%.0f  (want 427000)\n", rcCcpi, vC);
		...
		rcCa = SetTPParameter(PT_ACCEL, AXIS_C, ch3->Accel / 427000.0);
		printf("SET Z Vel/Accel rc=%d/%d  (%.3f in/s, %.3f in/s^2)\n",
		       rcZv, rcZa, ch2->Vel/180000.0, ch2->Accel/180000.0);
		printf("SET C Vel/Accel rc=%d/%d  (%.3f in/s, %.3f in/s^2)\n",
		       rcCv, rcCa, ch3->Vel/427000.0, ch3->Accel/427000.0);
		...
		// of Max Vel. Standard: Z=quill 0.3 in/s (18 IPM), C=knee 0.08 in/s
		// (4.8 IPM). Values chosen to match Jim's previous manual settings.
		rcZj = SetTPParameter(PT_JOG_VEL, AXIS_Z, 0.3);
		rcCj = SetTPParameter(PT_JOG_VEL, AXIS_C, 0.08);
		printf("SET Z JogVel rc=%d (0.3 in/s)  C JogVel rc=%d (0.08 in/s)\n",
		       rcZj, rcCj);
		...
		if (vZ2 != vZ || vC2 != vC)
			printf(">>> DELAYED GET DIFFERS: Z was %.0f now %.0f, C was %.0f now %.0f\n",
			       vZ, vZ2, vC, vC2);
		printf(">>> VERIFY Tool Setup | Trajectory Planner shows Z=180000 C=427000 plus per-axis Vel/Accel\n");

		// Screen "STANDARD init LOADED" indicator, deferred to here so it
		// only fires once TP setup is complete (see comment above). Made
		...
		// the screen shows a distinct TP FAIL message the operator can't miss.
		{
			...
			if (rcZcpi == 0 && rcCcpi == 0 &&
			    ...
			    rcZj == 0 && rcCj == 0 &&
			    vZ2 == 180000.0 && vC2 == 427000.0)
			{
				sprintf(_s, "STANDARD init LOADED");
				TP_verified = 1;
			}
			else
			{
				sprintf(_s, "STANDARD TP FAIL!!");
				TP_verified = 0;
			}
	...
	SetUserDataDouble(54, 1.0);	// Standard: quill on ch2, knee on ch3
	SetUserDataDouble(58, 1.0);	// pendant init identity -> LCD banner name: 1 = Standard  (var 58; NOT 56 = TP_HOLD_REQ)
